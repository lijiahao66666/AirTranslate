...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    _loadData();
    _pollTimer =
        Timer.periodic(const Duration(seconds: 8), (_) => _refreshJobs());
  ...
  Future<void> _download(Job job) async {
    try {
      ...
      if (Platform.isAndroid || Platform.isIOS) {
        ...
        if (!mounted) return;
        _showInfo('下载完成，文件已保存到本地：${file.path}');
      } else {
        await launchUrl(Uri.parse(url), mode: LaunchMode.externalApplication);
      }
    } catch (e) {
  ...
  Future<Directory> _resolveDownloadDirectory() async {
    if (Platform.isAndroid) {
      ...
      if (await publicDownload.exists()) return publicDownload;
  ...
  Future<void> _deleteJob(Job job) async {
    ...
    if (isAI && canRefund) {
      hint =
          '任务尚未开始翻译，删除后将自动退还 ${NumberFormat("#,###").format(job.pointsDeducted)} 积分。';
    } else if (isAI) {
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(hint,
                  style: TextStyle(fontSize: 13, color: dcs.onSurfaceVariant)),
                ...
                children: [
                  Expanded(
                    child: TextButton(
                      ...
                      style:
                          TextButton.styleFrom(minimumSize: const Size(0, 36)),
  ...
  void _showInfo(String msg, {Duration duration = const Duration(seconds: 5)}) {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
                  ...
                  child: Center(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('点击下方按钮新建并提交翻译任务',
                            style: TextStyle(
                                fontSize: 14, color: cs.outlineVariant)),
                      ],
                    ),
                  ),
                )
                ...
                SliverList(
                  delegate: SliverChildBuilderDelegate(
                    (context, index) {
                      ...
                      return JobCard(
                        job: job,
                        onDownload: job.progress?.isDone == true
                            ? () => _download(job)
                            : null,
                        onStart: job.progress?.state == 'READY'
                            ? () => _startJob(job)
                            : null,
